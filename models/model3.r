setwd("C:/Users/jonat/Desktop/project.deeptxtgen")
library(reticulate)
...
#Set parameters:
FLAGS <- flags(
  flag_integer('filters_cnn', 12),
  flag_integer('filters_lstm', 12),
  flag_numeric('reg1', 0.01),
  flag_numeric('reg2', 0.01),
  flag_numeric('batch_size',40),
  flag_numeric('maxlen', 150),
  flag_numeric('steps', 5),
  flag_numeric('embedding_dim', 500),
...
model <- keras_model_sequential(name = "m3") %>% 
...
model %>% save_model_hdf5("model3.h5", include_optimizer = TRUE)
